export const quotation = {
  reference: "STBS/SNP/RWH/21/05/2025",
  date: "21/05/2025",
  validity: "15 days from date of submission",
  client: ["SILVERON", "TILAK NAGAR, JAIPUR"],
  subject: "Price Offer for Borewell Construction",
  terms: [
    ["Taxes", "18% GST shall be charged extra on an actual basis."],
    ["Payment Terms", "75% advance payment and 25% upon project handover."],
    ["Electricity & Water", "The client shall provide electricity and water free of cost."],
    ["Storage", "A lockable storage space for materials and equipment must be provided by the client at the site. However, the safe handling of materials and equipment shall be our responsibility."],
    ["JCB Work", "All JCB-related work will be charged extra."],
    ["Validity", "This offer is valid for 15 days from the date of submission. After this period, the offer will be subject to our confirmation."],
  ],
  about: "Saini Tubewell Boring Service, established in 1992, has built a strong presence in borewell construction, borewell material supply, and rainwater harvesting services. We are committed to delivering reliable solutions and high-quality service to our customers. We also provide ongoing support to help ensure that supplied equipment operates efficiently and is serviced when required.",
  mission: "It’s the policy of the company to provide customers with products and services which achieve and sustain the highest possible quality standards. This statement is the basis of our management philosophy and reflects the commitment of every individual within the organization.",
  vision: "Excel in what we do and build a safe and secure environment for our community.",
  capabilities: ["Rain water harvesting work.", "Borewell 100mm to 400mm.", "Provide best quality of borewell material.", "Provide material at a reasonable rate.", "On time work."],
  clients: ["Brackparts Pvt. Ltd.", "Ashoka University", "Jupiter Laminator Pvt. Ltd.", "LT Overseas Pvt. Ltd. (Dawat Rice Mill)", "Amul Milk, Murthal", "Nidaan Hospital", "Devi Lal Park, Sonipat/Panipat", "Alaina Indane Gas", "Voestalpine VAE VKN India Pvt. Ltd.", "Maneta Pvt. Ltd.", "Avicreations", "Ajit Industries Pvt. Ltd.", "Shreeji International School", "Parker Mall, Kundli", "TDI City, Kundli", "ITEC Technopark, IIT Delhi Sonipat Campus", "Coral Drugs Pvt. Ltd.", "Rishi Laser Limited", "Marut Techno Tools Pvt. Ltd.", "A-One Tex Tech Pvt. Ltd.", "BigBasket, Sonipat Site", "O.P. Jindal Global University", "And many more.."],
  // The source DOCX stores the 13-row commercial table as an embedded graphic;
  // values are intentionally omitted until a readable source is provided.
  items: [],
} as const;
