...
export default async function NewQuotationPage({searchParams}:{searchParams:Promise<{clientId?:string}>}) { const session=await auth(); if(!session?.user) redirect("/admin/login"); const clients=await listClients(); const id=(await searchParams).clientId; const client=clients.find(x=>x.id===id); const base={...initialQuotation,quotationDate:new Date().toLocaleDateString("en-GB")}; const initial:QuotationState=client?{...base,clientId:client.id,client:{gstin:client.gstin,companyName:client.companyName,contactPerson:client.contactPerson,addressLine1:client.addressLine1,addressLine2:client.addressLine2,city:client.city,state:client.state,pinCode:client.pinCode,phone:client.phone,email:client.email}}:base; return <QuotationStudio initial={initial} clients={clients} backHref="/admin/quotations" backLabel="Quotations" />; }
